# Google Cloud Best Practices 
## Enterprise
- [Best practices for enterprise organizations](https://cloud.google.com/docs/enterprise/best-practices-for-enterprise-organizations)

## Architecture
- [GCP Architecture Framework](https://cloud.google.com/architecture/framework)

## Security
- [Google Cloud Security Foundations Blueprint Guide](https://services.google.com/fh/files/misc/google-cloud-security-foundations-guide.pdf)

## Compute 
### Compute Engine
Virtual machines, disks, and networking
- [Designing Robust Systems](https://cloud.google.com/compute/docs/tutorials/robustsystems)
- [Securely connecting to VM instances](https://cloud.google.com/solutions/connecting-securely)
- [Verifying the Identity of Instances](https://cloud.google.com/compute/docs/instances/verifying-instance-identity)
- [Best Practices for SQL Server Instances](https://cloud.google.com/compute/docs/instances/sql-server/best-practices)
- [Image Management Best Practices](https://cloud.google.com/solutions/image-management-best-practices)
- [Optimizing Application Latency with Load Balancing](https://cloud.google.com/load-balancing/docs/tutorials/optimize-app-latency)
- [Best Practices for Floating IP Addresses](https://cloud.google.com/solutions/best-practices-floating-ip-addresses)
- [Best Practices for Persistent Disk Snapshots](https://cloud.google.com/compute/docs/disks/snapshot-best-practices)
- [Best practices for 16 KB persistent disk and MySQL](https://cloud.google.com/compute/docs/tutorials/16kb-mysql-best-practices)

## Containers (GKE, Anthos, Cloud Run)
- [Best practices for building containers](https://cloud.google.com/solutions/best-practices-for-building-containers)
- [Container Blog Series](https://cloud.google.com/blog/search/?query=exploring%20container%20security&language=en&category=article&paginate=25&order=newest&hl=en)

### Cloud Functions
- [Tips & Tricks](https://cloud.google.com/functions/docs/bestpractices/tips)
- [Retrying Background Functions](https://cloud.google.com/functions/docs/bestpractices/retries)
- [Optimizing Networking](https://cloud.google.com/functions/docs/bestpractices/networking)
- [Testing and CI/CD](https://cloud.google.com/functions/docs/bestpractices/testing)

## Storage
- [Best practices for Cloud Storage](https://cloud.google.com/storage/docs/best-practices)

## Management Tools
- [Best Practices for Using Deployment Manager](https://cloud.google.com/deployment-manager/docs/best-practices/)

## Database Products
## Relational Options
### MySQL
- [Best practices](https://cloud.google.com/sql/docs/mysql/best-practices)

### AlloyDB
- [Best practices for using AlloyDB Auth](https://cloud.google.com/alloydb/docs/auth-proxy/best-practices)

## Relational and Non-Relational (NoSQL) Options
### Cloud Spanner
- [SQL best practices](https://cloud.google.com/spanner/docs/sql-best-practices)  
- [Bulk loading best practices](https://cloud.google.com/spanner/docs/bulk-loading)
- [Data Manipulation Language best practices](https://cloud.google.com/spanner/docs/dml-best-practices)
- [Schema design best practices](https://cloud.google.com/spanner/docs/schema-design)

## Non-Relational (NoSQL) Options
###  Cloud Datastore
- [Best Practices](https://cloud.google.com/datastore/docs/best-practices)

### Cloud Firestore
- [Best Practices](https://firebase.google.com/docs/firestore/best-practices)

## AI & Machine Learning Products
### Model Training
- [Best practices for implementing machine learning on Google Cloud](https://cloud.google.com/architecture/ml-on-gcp-best-practices)

### Dialogflow
- [Best practices](https://cloud.google.com/dialogflow/docs/best-practices)

### Cloud Speech-to-Text
- [Best practices](https://cloud.google.com/speech-to-text/docs/best-practices)

## Data Analytics Products
### BigQuery
- [Controlling costs](https://cloud.google.com/bigquery/docs/best-practices-costs)
- [Optimizing storage](https://cloud.google.com/bigquery/docs/best-practices-storage)
- [Avoiding SQL anti-patterns](https://cloud.google.com/bigquery/docs/best-practices-performance-patterns)
- [Managing query outputs](https://cloud.google.com/bigquery/docs/best-practices-performance-output)
- [Optimizing query computation](https://cloud.google.com/bigquery/docs/best-practices-performance-compute)
- [Optimizing communication between slots](https://cloud.google.com/bigquery/docs/best-practices-performance-communication)
- [Managing input data and data sources](https://cloud.google.com/bigquery/docs/best-practices-performance-input)

## Networking Products
### Cloud CDN
- [Best Practices for Content Delivery](https://cloud.google.com/cdn/docs/best-practices)
### Cloud VPN
- [Cloud VPN best practices](https://cloud.google.com/vpn/docs/resources/best-practices)
